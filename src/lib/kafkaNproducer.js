...
class KafkaProducer {
  ...
  sendMessagesAndWaitReport (args) {
    return new Promise((resolve, reject) => {
      ...
      try {
        const getTotalElapsed = hirestime()
        ...
        deliveryReportListener = (err, report) => {
          if (err) {
            this.log.warn({ uuid: args.uuid, err: err }, 'Error sending message to Kafka')
          ...
          if (deliveryCheck.length === 0) {
            ...
            const ms = getTotalElapsed(hirestime.MS)
        ...
        const timeout = setTimeout(() => {
          ...
          this.log.warn({ uuid: args.uuid }, message)
        ...
        this.promNbError.inc()
        this.log.warn({ uuid: args.uuid, err: err }, 'Error sending messages to Kafka')
        this.producer.removeListener('delivery-report', deliveryReportListener)
        this.producer.removeListener('event.error', errorListener)
        reject(err)
      }
