...
class KafkaConsumer {
  constructor (container) {
    ...
    this.log.trace({ config: consumerConfig }, 'Creating kafka consumer')
  ...
  connect (topics, uuid = Uuid()) {
    ...
    this.consumer.on('event.error', error => {
      this.log.error({ uuid: this.kafkaGlobalUuid, err: error }, `Error on Kafka listener: ${error.message}`)
    })
    ...
    return new Promise((resolve, reject) => {
      ...
      this.consumer.connect(
        ...
        (err, metadata) => {
          if (err) {
            this.log.error({ uuid: this.kafkaGlobalUuid, err }, 'Consumer error')
            return reject(err)
          } else {
