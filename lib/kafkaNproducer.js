...
class KafkaProducer {
  constructor (container) {
    ...
    this.log.info({ config: this.producerConfig }, 'Creating kafka producer')
  ...
  disconnect () {
    return new Promise((resolve, reject) => {
      ...
      this.producer.once('disconnected', arg => {
        this.log.info(
          { uuid: this.kafkaGlobalUuid },
          'producer disconnected. ' + JSON.stringify(arg)
        )
      ...
      try {
        ...
        deliveryReportListener = (err, report) => {
          ...
          if (deliveryCheck.length === 0) {
            ...
            this.log.info(
              { uuid: args.uuid },`${nbEvents} events send in ${ms} ms / avg per event = ${ms /
                nbEvents} ms`
            )
        ...
        errorListener = error => {
          console.error(error)
          this.log.error(
            { uuid: args.uuid ? args.uuid : this.kafkaGlobalUuid, err: error },
